            ...
            GoogleTokenInfo tokenInfo = restClient.post()
                ...
                .onStatus(HttpStatusCode::is4xxClientError, (request, response) -> {
                    throw new OAuthClientException(
                        ...
                        "Google 액세스 토큰 요청 중 4xx 에러: " + response.getStatusCode()
                    );
                })
                .onStatus(HttpStatusCode::is5xxServerError, (request, response) -> {
                    throw new OAuthClientException(
                        ResponseCode.BAD_GATEWAY_ERROR,
                        "Google 액세스 토큰 요청 중 5xx 에러: " + response.getStatusCode()
                    );
                })
        ...
        try {
            GoogleUserInfo userInfo = restClient.get()
                ...
                .onStatus(HttpStatusCode::is4xxClientError, (request, response) -> {
                    throw new OAuthClientException(
                        ResponseCode.BAD_REQUEST_ERROR,
                        "Google 사용자 정보 요청 중 4xx 에러: " + response.getStatusCode()
                    );
                })
                .onStatus(HttpStatusCode::is5xxServerError, (request, response) -> {
                    throw new OAuthClientException(
                        ResponseCode.BAD_GATEWAY_ERROR,
                        "Google 사용자 정보 요청 중 5xx 에러: " + response.getStatusCode()
                    );
                })
